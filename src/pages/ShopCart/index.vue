...
//type为了区分这三个元素
        ...
        switch(type){
         ...
         disNum = cart.skuNum > 1 ? -1 : 0;
          break;
          case "change":
            //用户输入进来的最终量，非法的,带给服务器为0
            if(isNaN(disNum) || disNum<1){
              disNum = 0
            }else{
              // 属于正常情况(小数取整)，带给服务器变化量，用户输入进来的量 - 产品的起始的量
   ...
   async upDateChecked(cart,event){
     ...
     try {
      ...
      await this.$store.dispatch('updatedCheckedById',{skuId:cart.skuId,isChecked});
      this.getDate();
     } catch (error) {
      // 如果失败提示
      alert(error.message)
    ...
    computed:{
      ...
      isAllCheck(){
        //遍历数组里面的元素，只有全部元素isChecked属性都为1===>真
        // 只要有一个不是1就是false
